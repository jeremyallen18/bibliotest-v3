...
$libros = [];
$stmt = $conexion->prepare("SELECT id, titulo, imagen, descripcion FROM libros");
if ($stmt === false) {
        ...
        <?php if (!empty($libros)): ?>
          ...
          <div class='book-card' onclick="window.location.href='detalle_libro.php?id=<?= $row['id'] ?>'">
            <div class='book-image-container'>
              ...
              <img src='<?= htmlspecialchars($row['imagen']) ?>' class='book-image' alt='<?= htmlspecialchars($row['titulo']) ?>'>
            </div>
            <div class='book-info'>
